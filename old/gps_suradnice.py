...
result = database.find_near_gps(17.36036860521739, 48.2159461036635, 1000)
...
def main():
    ...
    min = minmax_z["min"] * pc_scale[2]
    max = minmax_z["max"] * pc_scale[2]
    for point in points:
        point[2] = point[2] * pc_scale[2]
        point[2] = point[2] + pc_offset[2]
        point[2] = point[2] - min
        point[2] = point[2] / (max-min)

    for point in points:
        point[:2] = point[:2] - mid[:2]
        point[:2] = point[:2] * 10
    print(points[:100,:3])
    ...
    projection = pyrr.matrix44.create_perspective_projection_matrix(
        fovy=45.0, aspect=work_width / work_height, near=0.001, far=10000.0, dtype=np.float32
    )

    # Initialize camera parameters.
    # camera_pos = np.array(middle + np.array([-0.02, 0.0, 0.0]), dtype=np.float32)
    camera_pos = np.array(points[0,:3], dtype=np.float32)
    ...
    while not glfw.window_should_close(window):
        ...
        # --- Keyboard movement ---
        camera_pos = process_keyboard(window, camera_pos, camera_front, camera_up, speed=0.1, delta_time=delta_time)
        #load_points(middle, camera_pos, threshold=20.0, radius=30.0, scale_factor=scale_factor)
